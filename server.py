...
# Keep the self-contained fallback small enough to fit common serverless
# request limits after JSON/Base64 overhead. The initial preview endpoint
# still uses MAX_INGEST_BODY_BYTES; this cap applies only to the copy echoed
# back on a later commit when the original draft is unavailable.
MAX_SOURCE_PAYLOAD_BYTES = 4 * 1024 * 1024
...
def _source_payload_for_rebuild(payload: Dict[str, Any], draft_id: str) -> Optional[Dict[str, Any]]:
    """Validate and bind a client-carried upload used for draft recovery.

    The payload is intentionally reduced to the same fields accepted by the
    ingest endpoint. Candidate notes, paths, and generated metadata are never
    accepted from the client; they are regenerated by ``_prepare_knowledge_draft``.
    """

    if "source_payload" not in payload:
        return None
    raw = payload.get("source_payload")
    ...
    try:
        encoded_size = len(json.dumps(raw, ensure_ascii=False, separators=(",", ":")).encode("utf-8"))
    except (TypeError, ValueError, UnicodeError) as exc:
        raise KnowledgeIngestError(
            HTTPStatus.BAD_REQUEST,
            "invalid_source_payload",
            "source_payload must contain JSON values",
        ) from exc
    if encoded_size > MAX_SOURCE_PAYLOAD_BYTES:
        raise KnowledgeIngestError(
            HTTPStatus.REQUEST_ENTITY_TOO_LARGE,
            "source_payload_too_large",
            "source_payload is too large to recover this draft",
        )
    unknown = sorted(set(raw) - _SOURCE_PAYLOAD_FIELDS)
    if unknown:
        raise KnowledgeIngestError(
            HTTPStatus.BAD_REQUEST,
            "invalid_source_payload",
            "source_payload contains unsupported fields",
        )
    supplied_id = _requested_draft_id(raw)
    if supplied_id is not None and supplied_id != draft_id:
        raise KnowledgeIngestError(
            HTTPStatus.CONFLICT,
            "draft_id_conflict",
            "source_payload draft_id does not match the commit draft",
        )
    rebuilt = {key: raw[key] for key in _SOURCE_PAYLOAD_FIELDS if key in raw}
    # Bind the recovery input to the already validated outer request ID. This
    # also prevents a nested preview_id from silently selecting another draft.
    rebuilt.pop("preview_id", None)
    rebuilt["draft_id"] = draft_id
    return rebuilt
...
def _commit_knowledge_draft(root: Path, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with KNOWLEDGE_INGEST_LOCK:
        ...
        if draft is None:
            # Vercel can send preview and commit to different ephemeral
            # containers. Rebuild the deterministic draft from the original
            # upload carried by the browser, when available.
            source_payload = _source_payload_for_rebuild(payload, requested_id)
            if source_payload is not None:
                draft = _prepare_knowledge_draft(root, source_payload)
        if draft is None:
